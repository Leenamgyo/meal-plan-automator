#!/usr/bin/env node
/**
 * SQLite 로컬 DB 더미 데이터 시드 스크립트 (GraphQL 기반)
 * 사용법: npm run seed [URL]   또는   npx tsx scripts/seed.ts [URL]
 * 기본 URL: http://127.0.0.1:3737/graphql
 *
 * 기존 데이터를 삭제 후 다시 생성한다 (테스트용).
 * meal_data는 MealEntry 형식 { name, category_id, color }로 생성된다.
 */

const ENDPOINT = process.argv[2] || 'http://127.0.0.1:3737/graphql';
...
const rawMenus: Record<string, string[]> = {
  밥: [
...
async function deleteAllCategories(): Promise<number> {
  const { categories: rows } = await gql<{ categories: Array<{ id: number }> }>(
    `query { categories { id } }`,
  );
  let n = 0;
  for (const r of rows) {
...
async function deleteAllMenuItems(): Promise<number> {
  const { menuItems } = await gql<{ menuItems: Array<{ id: number }> }>(
    `query { menuItems { id } }`,
  );
  let n = 0;
...
async function deleteAllMealData(): Promise<number> {
  const { mealData } = await gql<{ mealData: Array<{ date: string }> }>(
    `query { mealData { date } }`,
  );
  let n = 0;
  for (const r of mealData) {
    await gql<{ deleteMealDataByDate: boolean }>(
      `mutation($date: String!) { deleteMealDataByDate(date: $date) }`,
      { date: r.date },
    );
    n++;
  }
...
async function main(): Promise<void> {
  ...
  // Health check
  try {
    await gql(`query { categories { id } }`);
    console.log('✅ 로컬 GraphQL 서버 연결 성공\n');
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('❌ 로컬 GraphQL 서버에 연결할 수 없습니다:', msg);
    console.error('먼저 앱을 실행하세요: npm start');
    process.exit(1);
  }

  // Step 1: 기존 데이터 삭제 (의존 관계 역순)
  console.log('🧹 기존 데이터 삭제 중...');
  console.log(`  [meal-data]   🗑️  ${await deleteAllMealData()}개 삭제`);
  console.log(`  [menu-items]  🗑️  ${await deleteAllMenuItems()}개 삭제`);
  console.log(`  [categories]  🗑️  ${await deleteAllCategories()}개 삭제`);

  // Step 2: 카테고리 생성
  console.log('\n📁 카테고리 생성 중...');
  const catMap: Record<string, { id: number; color: string }> = {};
  for (const cat of categories) {
    try {
      const { createCategory } = await gql<{
        createCategory: { id: number; name: string; color: string };
      }>(
      ...
      catMap[cat.name] = { id: createCategory.id, color: cat.color };
      console.log(`  ✅ ${cat.name} → id: ${createCategory.id}`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`  ❌ ${cat.name}: ${msg}`);
    }
  }

  // Step 3: 메뉴 생성
  console.log('\n🍽️  메뉴 생성 중...');
  const menuPool: Record<string, MealEntry[]> = {};
  let total = 0;
  let success = 0;
  ...
  for (const [catName, menus] of Object.entries(rawMenus)) {
    ...
    if (!catInfo) {
      console.log(`  ⚠️  카테고리 "${catName}" ID 없음, 건너뜀`);
      continue;
    }
    menuPool[catName] = [];
    ...
    for (const name of menus) {
      total++;
      try {
        await gql<{ createMenuItem: { id: number } }>(
          `mutation($input: MenuItemCreateInput!) {
            createMenuItem(input: $input) { id }
          }`,
          {
            input: {
              name,
