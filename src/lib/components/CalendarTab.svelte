<script lang="ts">
    ...
    async function autoGenerateMeal() {
        ...
        try {
            ...
            const categoryScores = calculateMenuScores(
                menuItems,
                categories,
                mealDataNames,
                targetDate,
...
<div class="flex h-full overflow-hidden relative">
                ...
                {#each calendarDays as cd}
                    ...
                    >
                        ...
                        {#if isToday(cd) && cellMeals.length === 0}
                            <div
                                class="absolute -top-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all pointer-events-none"
                            >
                                <div
                                    class="bg-primary-container text-on-primary-container px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter shadow-lg flex items-center gap-1 whitespace-nowrap"
                                >
                                    <span
                                        class="material-symbols-outlined"
    ...
    {#if showPanel}
        ...
        >
            ...
            <div
                class="px-6 py-4 border-t border-surface-container-high flex items-center justify-between"
            >
                <p class="text-[10px] text-on-surface-variant font-medium">
                    <span class="font-bold text-primary">더블클릭</span>으로
                    메뉴 배정/삭제
                </p>
                <button
