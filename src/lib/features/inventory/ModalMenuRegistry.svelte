<script lang="ts">
    import { createMenuItem } from "$lib/features/menu/api";
    import { suggestIngredients } from "$lib/features/ai/mealService";
    import { geminiKey, aiIngredientsEnabled, showSuccess } from "$lib/shared/stores";
    ...
    let selectedIcon = $state("rice_bowl");
    let menuName = $state("");
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (saved) {
                ...
                onsaved({ id: saved.id || Date.now(), ...data });
            }
        } finally {
            isSaving = false;
        }
                        ...
                        {#each ingredients as ing}
                            <span class="px-2 py-1 bg-surface-container-low text-on-surface-variant rounded-md text-[10px] font-bold flex items-center gap-1">
                                ...
                                <button
                                    ...
                                    onclick={() => (ingredients = ingredients.filter((t) => t !== ing))}
                                >
